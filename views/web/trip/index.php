...
<div class="bg1">
    <div class="con">
        <h2 class="list-title s18 n">共找到 <font class="orange"><?= $total ?></font> 处楼盘
            <a href="/web/trip/"><font style="font-size:12px;float:right;margin-left:5px;">清空条件</font><span style="float:right;width:13px;height:13px;background-image:url(/static/web/images/clear.png)" ></span></a>
        </h2>
        <ul class="house-list">
            <?php foreach ($loupan_list as $item): ?>
                <li>
                    ...
                    <div class="list-info fl">
                        <h3 class="s18"><a href="/web/trip/detail?id=<?= $item->id ?>"><?= $item->name ?></a></h3>
                        <p><?= $item->address ?></p>
        ...
        if (!empty($pages)) {
            echo '
        <div class="page" >
            <a href = "/web/trip/?page=' . $pages['pre'] . '" title = "上一页" class="page-prev" > 上一页</a >
            <a href = "/web/trip/?page=' . $pages['next'] . '" title = "下一页" class="page-next" > 下一页</a >
        </div > ';
        } ?>
    </div>
</div>
